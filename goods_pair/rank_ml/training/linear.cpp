		...
		for (s=0; s<l; s++)
		{
			...
			if(inner_iter > 0) // update w
			{
				...
				while (xi->index != -1)
				{
					...
					w[xi->index-1] += sign*(z-alpha_old)*yi;
					xi++;
				}
			}
		}
	...
	double Cp, double Cn)
{
	...
	while(newton_iter < max_newton_iter)
	{
		...
		info("iter #%3d cycles with minimized eps[%f - %f]\n", newton_iter, Gnorm1_new/Gnorm1_init, eps);
		if(Gnorm1_new <= eps*Gnorm1_init)
			break;
		...
		while(iter < max_iter)
		{
			...
			iter++;
			info("iter %3d #CD cycles %d with change rate [%f]\n", newton_iter, iter, QP_Gnorm1_new/Gnorm1_init);
...
//		}
	...
	info("Minimum Objective value = %lf\n", v/all_lable_num);
	info("#nonzeros/#features = %d/%d\n", nnz, w_size);
...
static void transpose(const problem *prob, feature_node **x_space_ret, problem *prob_col)
{
	...
	prob_col->l = l; //instance dimension
	prob_col->n = n; //feature dimesion
...
static void train_one(const problem *prob, const parameter *param, double *w, double Cp, double Cn)
{
	...
	switch(param->solver_type)
	{
		...
		case L1R_LR:
		{
			...
			//feature_node *x_space = NULL;
			//transpose(prob, &x_space ,&prob_col);
			solve_l1r_lr(prob, w, primal_solver_tol, Cp, Cn);
			//delete [] prob_col.y;
			//delete [] prob_col.x;
			//delete [] prob_col.W;
			//delete [] x_space;
...
model* train(const problem *prob, const parameter *param)
{
	...
	else if(param->solver_type == L1R_LR)
	{
		model_->w = Malloc(double, w_size);
		model_->nr_class = 2;
		model_->label = Malloc(int, 2);
		model_->label[0] = 1;
		model_->label[1] = -1;
		train_one(prob, param, &model_->w[0], param->C, param->C);
	}
	else
	{
		int nr_class;
