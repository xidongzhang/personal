...
argvDict={}
#sign="train"
basePath="/user/chongweishen/personal/rank"

def userActionMerge():
    #tempOutput="/user/chongweishen/personal/rank/ctr_study_%s/action_merge"
    tempOutput="%s/ctr_study_%s/action_merge"%(basePath,sign)
    ...
    job.setMapstr("python MergeMapper.py %s %s %s %s"%(dayLimit,dayLast,sample,personal))
...
def userGoods():
    ...
    tempOutput="%s/ctr_study_%s/user_goods_info"%(basePath,sign)
    hour=None
    if(argvDict.has_key("hour")):
        hour=argvDict["hour"]
    job = HadoopJobProducer()
    ...
    job.addInput("%s/ctr_study_%s/action_merge"%(basePath,sign),"",partName)
...
def dataFilt():
    ...
    tempOutput="%s/ctr_study_%s/final_data"%(basePath,sign)
    hour=None
    colLimit=4
    ...
    if(argvDict.has_key("hour")):
        hour=argvDict["hour"]
    job = HadoopJobProducer()
    job.setJobName("hash_fea")
    job.setMapstr("python HashMapper.py")
    #job.setReducestr("python FiltReducer.py %s"%colLimit)
    job.setJobPriority("NORMAL")
    job.addFile(["HashMapper.py","discretization.config","utils.py","/hadoop/hive/lib/hive-exec-0.14.0.jar"])
    partName="*"
    if(argvDict.has_key("part")):
        partName=argvDict["part"]
    
    #job.addInput("/user/chongweishen/personal/rank/ctr_study_train/user_goods_info/*","",partName)
    job.addInput("%s/ctr_study_%s/user_goods_info/*"%(basePath,sign),"",partName)
