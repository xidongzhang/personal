...
def getSubtiTestPair():
    ...
    hql = '''
    hive -e "
        drop table tmp.recommend_pair_subti;
        set hive.exec.compress.output=false;
        create table tmp.recommend_pair_subti as select tid_scenario['twitter_id'] as tid1, list_scenario['source_tid'] as tid2, 'subti' as sample_type, '-' as tag1 , '-'     as tag2, 1 as frequence from tmp.dm_mobctr_useractionevent_txt where dt >= '%s' and dt <= '%s' and list_scenario['query']='index-similar' and action_id = 'statistics-single_twitter' union all select tid_scenario['twitter_id'] as tid1, list_scenario['source_tid'] as tid2, 'subti_no' as sample_type, '-' as tag1 , '-' as tag2, 1 as frequence from tmp.dm_mobctr_useractionevent_txt where dt >= '%s' and dt <= '%s'and list_scenario['query']='index-similar' and action_id = 'search-tag_poster';
    "
    '''
...
def mergeFeaJob1():
    rootPath=argvDict["root_path"]
    tempOutput="%s/fea_data/test_subti_pair_merge_fea1"%(rootPath)
    job = HadoopJobProducer()
    job.setJobName("pair_merge_fea1")
    job.setMapstr("python MergeFea1SubtiMapper.py")
    job.setReducestr("python MergeFea1Reducer.py")
    job.setJobPriority("HIGH")
    job.addFile(["MergeFea1SubtiMapper.py","MergeFea1Reducer.py","goods_meta","utils.py","/hadoop/hive/lib/hive-exec-0.14.0.jar"])
    partName="*"
    ...
    day = argvDict["day"]
    job.addInput("/user/hive/warehouse/tmp.db/recommend_pair_subti/*","",partName)
...
def mergeFeaJob2():
    rootPath=argvDict["root_path"]
    tempOutput="%s/fea_data/test_subti_pair_merge_fea2"%(rootPath)
    job = HadoopJobProducer()
    job.setJobName("pair_merge_fea2")
    job.setMapstr("python MergeFea2SubtiMapper.py")
    job.setReducestr("python MergeFea2Reducer.py")
    job.setJobPriority("HIGH")
    job.addFile(["MergeFea2SubtiMapper.py","MergeFea2Reducer.py","goods_meta","utils.py","/hadoop/hive/lib/hive-exec-0.14.0.jar"])
    partName="*"
    if(argvDict.has_key("part")):
        partName=argvDict["part"]
    cmd="hadoop fs -rmr %s"%(tempOutput)
    ...
    day = argvDict["day"]
    job.addInput("%s/fea_data/test_subti_pair_merge_fea1"%(rootPath),"",partName)
    job.addInput("%s/nlp/goods_fea_tag"%(rootPath),"",partName)
    job.setReduceNum(800)
    job.setOutput(tempOutput)
    job.addOtherInfomation("-jobconf num.key.fields.for.partition=1")
...
def formatFea():
    rootPath=argvDict["root_path"]
    tempOutput="%s/fea_data/test_subti_format_fea"%(rootPath)
    job = HadoopJobProducer()
    ...
    job.addInput("%s/fea_data/test_subti_pair_merge_fea2"%(rootPath), day, partName)
    job.setReduceNum(800)
    job.setOutput(tempOutput)
    job.addOtherInfomation("-jobconf mapred.job.map.capacity=800")
    job.addOtherInfomation("-jobconf mapred.job.reduce.capacity=800")
...
def combineFea():
    rootPath=argvDict["root_path"]
    sign = argvDict["sign"]
    tempOutput="%s/fea_data/test_subti_comb_fea"%(rootPath)
    ...
    day = argvDict["day"]
    job.addInput("%s/fea_data/test_subti_format_fea"%(rootPath), day, partName)
    job.setReduceNum(800)
    job.setOutput(tempOutput)
    job.addOtherInfomation("-jobconf mapred.job.map.capacity=800")
    job.addOtherInfomation("-jobconf mapred.job.reduce.capacity=800")
